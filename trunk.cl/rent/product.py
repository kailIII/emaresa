...
class Product(osv.osv, ExtendedOsv):
    ...
    def default_time_unity(self, cr, uid, context=None):
        ...
        if not unity:
		print "It seems that there isn't a reference unity in the 'Duration' UoM category. \
				Please check that the category exists, and there's a reference unity."
			
#		raise osv.except_osv("Error","It seems that there isn't a reference unity in the 'Duration' UoM category. \
 #                           Please check that the category exists, and there's a refernce unity.")
    ...
    _columns = {
        ...
        'rent_time_unity': fields.many2one('product.uom', 'Rent Time Unity', domain=[('category_id.name', '=', 'Duration')],
