...
class purchase_cost_order(orm.Model):
    ...
    def _get_expenses(self, cr, uid, ids, context=None):
        result = {}
        for line in self.pool.get('purchase.cost.order.expense').browse(cr,
                                                                        uid,
                                                                        ids,
                                                                        context=context):
            result[line.order_id.id] = True
    ...
    _columns = {
                ...
                'expense_amount': fields.function(_amount_expense,
                                                  ...
                                                  store={
                                                         'purchase.cost.order.expense': (_get_expenses,
                                                                                         None,
                                                                                         20
                                                                                         ),
                                                         }
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        return super(purchase_cost_order, self).create(cr,
                                                       uid,
                                                       vals,
                                                       context=context)
    ...
    def action_draft2calculated(self, cr, uid, ids, context=None):
            ...
            for expense in order.expense_line:
                if expense.expense_amount == 0.0:
                    raise orm.except_orm(
                        _('ERROR EXPENSE!'),
                        _('Please enter amount for this expense: "%s" (amount:%d)') % \
            ...
            for expense in order.expense_line:
                if expense.type_id.calculation_method == 'amount':
                    for line in order.cost_line:
                        ...
                        self.pool.get('purchase.cost.order.line.expense').create(
                                                                cr,
                                                                uid,
                                                                res,
                                                                context=context
        ...
        for line in order.cost_line:
            ...
            line.write({'cost_ratio': cost_ratio,
        ...
        for order in self.browse(cr, uid, ids, context=context):
            for line in order.cost_line:
                ...
                product_obj.write(cr,
                                  uid,
                                  [line.product_id.id],
                                  {'standard_price': line.standard_price_new}
                                  )
...
class purchase_cost_order_line(orm.Model):
    ...
    _columns = {
                ...
                'amount_weight': fields.function(
                                               _amount_line,
                                               type='float',
                                               store={
                                                      'purchase.cost.order.line':(lambda self,
                                                                                  cr,
                                                                                  uid,
                                                                                  ids,
                                                                                  ctx=None: ids,
                                               ...
                                               string='Line Gross Weight',
                                               digits_compute=dp.get_precision('Stock Weight'),
                                               help="The line gross weight in Kg."
                                               ),
                'amount_weight_net': fields.function(
                                                     _amount_line,
                                                     type='float',
                                                     store={
                                                            'purchase.cost.order.line':(lambda self,
                                                     ...
                                                     string='Line Net Weight',
                                                     digits_compute=dp.get_precision(
                                                                                'Stock Weight'
                                                                                ),
                                                help="The line net weight in Kg."
                                                ),
                'amount_volume': fields.function(
                                                 _amount_line,
                                                 type='float',
                                                 store={
                                                        'purchase.cost.order.line':(lambda self,
