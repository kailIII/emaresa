...
class account_journal(osv.osv):
	...
	_columns = {
		'invoice_sequence_id': fields.many2one('ir.sequence', 'Invoice Sequence',\
				domain="[('company_id','=',company_id)]", help="The sequence used for invoice numbers in this journal.")
	}

	def _check_company(self, cr, uid, ids, context=None):
		for journal in self.browse(cr, uid, ids, context=context):
			if journal.invoice_sequence_id and journal.invoice_sequence_id.company_id != journal.company_id:
				raise osv.except_osv(_('Invoice Sequence Error'),\
							_("The company of invoice sequence does not match journal's company."))
		return True
