...
class rent_order_line_make_invoice(osv.osv_memory):
	_name = "rent.order.line.make.invoice"
	_description = "Rent OrderLine Make_invoice"
	...
	def make_invoices(self, cr, uid, ids, context=None):
		...
		def make_invoice(order, lines):
			...
			inv = {
				...
				'origin': order.name,
				'type': 'out_invoice',
				'reference': "P%dRENT%d" % (order.partner_id.id, order.id),
				'account_id': a,
		...
		sales_order_line_obj = self.pool.get('rent.order.line')
		sales_order_obj = self.pool.get('rent.order')
		wf_service = netsvc.LocalService('workflow')

		for line in rent_order_line_obj.browse(cr, uid, context.get('active_ids', []), context=context):
